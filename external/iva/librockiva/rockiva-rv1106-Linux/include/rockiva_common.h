...
typedef enum {
    ...
    ROCKIVA_DET_MODEL_CLS9,           /* 检测类别：人形、人脸、机动车、车牌、非机动车、宠物、人头、快递包裹 */
    ROCKIVA_DET_MODEL_PV              /* 检测类别：人形、车辆 */
} RockIvaDetModel;
...
typedef enum {
    ...
    ROCKIVA_CAMERA_TYPE_VIDEO_STITCHING = 3,   /* 双目视频拼接 */
} RockIvaCameraType;
...
typedef struct {
    ...
    uint16_t wstride;                    /* 图像宽度对齐 */
    uint16_t hstride;                    /* 图像高度对齐 */
