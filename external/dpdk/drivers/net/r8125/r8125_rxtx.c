...
extern uint32_t		r8125_gbd_addr_b_p[5];
extern uint32_t		r8125_gbd_addr_r_p[5];
extern uint32_t		r8125_gbd_addr_t_p[5];
extern uint32_t		r8125_gbd_addr_x_p[5];

extern void			*r8125_gbd_addr_b_v[5];
extern void			*r8125_gbd_addr_t_v[5];
extern void			*r8125_gbd_addr_r_v[5];
extern void			*r8125_gbd_addr_x_v[5];
extern uint64_t r8125_base_hw_addr;
...
void
rtl8125_rx_queue_release(void *rx_queue)
{
	PMD_INIT_FUNC_TRACE();
...
uint64_t
rtl8125_get_rx_port_offloads(struct rte_eth_dev *dev __rte_unused)
{
	uint64_t offloads;
                       ...
                       struct rte_mempool *mb_pool)
{
	...
	if (dev->data->rx_queues[queue_idx] != NULL) {
		rtl8125_rx_queue_release(dev->data->rx_queues[queue_idx]);
	...
	int index;
	index = abs((uint64_t)hw->mmio_addr - r8125_base_hw_addr) / 0x5000;
	printf("index: %d, mmio_addr: 0x%lx, r8125_base_hw_addr: 0x%lx\n", index, hw->mmio_addr, r8125_base_hw_addr);
	rxq->hw_ring_phys_addr = r8125_gbd_addr_r_p[index];
	rxq->hw_ring = (struct rtl8125_rx_desc *)r8125_gbd_addr_r_v[index];
	printf("hw rx ring size: %d:%ld[0x%lx:%p]\n",
						size,
						sizeof(struct rtl8125_rx_desc),
...
rtl8125_rx_init(struct rte_eth_dev *dev)
{
	...
	//max_rx_pkt_size = dev->data->dev_conf.rxmode.max_rx_pkt_len + 2 * VLAN_TAG_SIZE;
	max_rx_pkt_size = dev->data->dev_conf.rxmode.mtu + 2 * VLAN_TAG_SIZE;
	printf("%s: mtu %d\n", __func__, dev->data->dev_conf.rxmode.mtu);
...
void
rtl8125_tx_queue_release(void *tx_queue)
{
	PMD_INIT_FUNC_TRACE();

	if (tx_queue != NULL) {
		struct rtl8125_tx_queue *txq = (struct rtl8125_tx_queue *)tx_queue;
...
uint64_t
rtl8125_get_tx_port_offloads(struct rte_eth_dev *dev __rte_unused)
{
                       ...
                       const struct rte_eth_txconf *tx_conf)
{
	...
	if (dev->data->tx_queues[queue_idx] != NULL) {
		rtl8125_tx_queue_release(dev->data->tx_queues[queue_idx]);
	...
	index = abs((uint64_t)hw->mmio_addr - r8125_base_hw_addr) / 0x5000;
	printf("index: %d, mmio_addr: 0x%lx, r8125_base_hw_addr: 0x%lx\n", index, hw->mmio_addr, r8125_base_hw_addr);
	txq->hw_ring_phys_addr = r8125_gbd_addr_t_p[index];
	txq->hw_ring = (struct rtl8125_tx_desc *)r8125_gbd_addr_t_v[index];
	printf("hw tx ring size: %d:%ld[0x%lx:%p]\n",
				size,
				sizeof(struct rtl8125_tx_desc),
...
rtl8125_free_queues(struct rte_eth_dev *dev)
{
	...
	rtl8125_rx_queue_release(dev->data->rx_queues[0]);
	dev->data->rx_queues[0] = 0;
	dev->data->nb_rx_queues = 0;

	rtl8125_eth_dma_zone_free(dev, "tx_ring", 0);
	rtl8125_tx_queue_release(dev->data->tx_queues[0]);
